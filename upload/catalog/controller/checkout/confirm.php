...
class Confirm extends \Opencart\System\Engine\Controller {
	public function index(): string {
		...
		$this->session->data['message_order'] = $this->messageW($order_data , $total);
	...
	public function messageW($order_data , $total): string {

					//create messaage order detaials by  social link 


					$order_info_array = $order_data;
					$new_line = '%0a';
					$text_message = '';
				//	$whatsapp_link = 'https://api.whatsapp.com/send?phone=+967714589027&text=';
			
			// Customer Details
			$text_message .= 'مرحبا ' . $new_line
									  .'انا' .' '. $order_info_array['firstname'].' ' . $order_info_array['lastname'] . $new_line
									  .'رقم هاتفي هو ' . $order_info_array['telephone'] . $new_line
									  .'المحافظة => ' . $order_info_array['shipping_zone']. $new_line
									  . 'العنوان الاول => ' .$order_info_array['shipping_address_1'] . $new_line 
									  .'العنوان الثاني => ' .$order_info_array['shipping_address_2'] . $new_line ;
			
			
			// Products
			$text_message .= 'أريد شراء الاتي:' . $new_line;
			
			$allProdact = $order_info_array['products'];
			foreach ($allProdact as $key => $producted1) {
				//$text_message .= count($allProdact);
			
				$text_message .= '____________'. $key+1 .'______________' . $new_line;
				foreach( $producted1 as $key1 =>  $product1){
					
					
							if (is_array($product1) || is_object($product1))
									{
										  foreach ($product1 as $key2 => $product2){
										//	$text_message .= $key2 . '=>' . $product2;
									} //end for 3
								  } // end if
							else{
											  ...
											  switch ($key1) {
															  case "name":
														$text_message .=  "*اسم المنتج*". " => " . $product1 . $new_line;
																break;
															 case "model":
														$text_message .=  "*الموديل*". " => " . $product1 . $new_line;
															  break;
															 case "quantity":
														 $text_message .=  "*العدد*". " => " . $product1 . $new_line;
																break;
														   //  case "product_id":
													 //	$text_message .=  "*الرابط*". " => " . $this->url->link('product/product', 'language=' . $this->config->get('config_language') . '&product_id=' . $product1 , true). $new_line;
															//   break;
															  case "total":
														   $text_message .=  "*الإجمالي*". " => " . $product1 . $new_line;
																 break;
			...
			//Shipping Details
			$text_message .= '__________________________' . $new_line;
			$text_message .= '*معلومات التوصيل*' . $new_line 
							.'طريقة الشحن => ' .$order_info_array['shipping_method'] . $new_line;
						//	.'تكلفة الشحن => '. $taxes . $new_line ;
			...
			// Payment Details
			$text_message .= '__________________________' . $new_line;
			$text_message .= '*معلومات الدفع*' . $new_line 
							.'طريقة الدفع => ' . $order_info_array['payment_method'] . $new_line 
							.'إجمالي المبلغ => *('.$total . ')*'. $new_line 
							.$order_info_array['payment_firstname'] .  $order_info_array['payment_lastname'] 
							.$order_info_array['payment_address_1'] .  $order_info_array['payment_address_2']
							.$order_info_array['payment_city']      . $order_info_array['payment_zone'];
			
				// Gift Voucher
				
			
				if($order_info_array['comment'] != ''){
					$text_message .= '__________________________' . $new_line;
					$text_message .= '*ملاحضات*' . $new_line ;
					$text_message .= $order_info_array['comment'];
				}
